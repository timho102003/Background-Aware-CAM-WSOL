...
class MutualExclusiveAggregator(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # print(f"mea_out shape: {mea_out.shape}")
...
class WSOLSystem(pl.LightningModule):
    ...
    def on_predict_start(self) -> None:
        ...
        self.weight_softmax_o = self.weight_softmax_o.unsqeeze(0) if len(
            self.weight_softmax_o) == 1 else self.weight_softmax_o
        self.weight_softmax_b = self.weight_softmax_b.unsqeeze(0) if len(
            self.weight_softmax_b) == 1 else self.weight_softmax_b

        def _feature_hook(module, input, output):
            self.feature_blobs.append(output)
        print("register feature hook")
        self.backbone._modules.get(
            "layer4").register_forward_hook(_feature_hook)
        self.predict_outpath = "./checkpoints/cam_out"
    ...
    def cal_stagger_loss(self, oo, ob, bo, bb, targets):
        ...
        tar_bo = torch.ones(bo.size()).scatter_(
            1, targets.cpu(), 0).to(bo.device)
        tar_ob = torch.zeros(ob.size(), device=ob.device)
        tar_bb = torch.ones(bb.size(), device=bb.device)
        loss_oo = self.criteria(oo, tar_oo)
        loss_bo = self.criteria(bo, tar_bo)
        loss_ob = self.criteria(ob, tar_ob)
        loss_bb = self.criteria(bb, tar_bb)
    ...
    def configure_optimizers(self):
        ...
        #     optimizer=optimizer, T_max=self.hparams.max_epoch)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        print(f"\npredict class: {idx[0].item()}")
        print(f"real class: {y.item()}")
        ...
        result_mix = heatmap_mix * 0.3 + img_show * 0.7
        result_bg = heatmap_bg * 0.3 + img_show * 0.7
        result_fg = heatmap_fg * 0.3 + img_show * 0.7
    ...
    def generate_cam(self, feature_conv, class_idx, outsize):
        ...
        cam_mix = torch.stack([cam_bg, cam_obj])
        cam_idx = torch.argmax(cam_mix, dim=0)
        cam = torch.where(cam_idx == 1, cam_obj, torch.zeros_like(cam_obj))
        cam_dict["mix"].append(_norm_and_resize(cam, outsize))
        cam_dict["obj"].append(_norm_and_resize(cam_obj, outsize))
        cam_dict["bg"].append(_norm_and_resize(cam_bg, outsize))
